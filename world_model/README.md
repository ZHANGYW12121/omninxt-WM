...
This directory is the Alienware-authoritative world-model source for OmniNxt.
In addition to the pinned R2-Dreamer base, it contains the current two-branch
ego/human factorized Dreamer implementation, goal-token action readout, sparse
observation attention, coupled imagination transition, Isaac dataset adapter,
reward model integration, and regression tests. See
[`docs/factorized_dreamer_final.md`](docs/factorized_dreamer_final.md) and
[`ego_env_human_factorized_dreamer_codex_guide.txt`](ego_env_human_factorized_dreamer_codex_guide.txt)
for the project-specific design.

Run the project regression suite from the repository root:
