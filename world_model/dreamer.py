...
class Dreamer(nn.Module):
    def __init__(self, config, obs_space, act_space):
        ...
        self.reward_components = RewardComponentHead(self.rssm.feat_size, int(config.reward_component_units))
    ...
    def _cal_grad(self, data, initial):
        ...
        if "reward_components" in data.keys():
            losses["rew_components"] = self.reward_components.loss(
                feat, to_f32(data["reward_components"])
            )
