...
class FactorizedBuffer(Buffer):
    """Replay buffer variant storing the Ego/Human latent branches."""
    ...
    def sample(self):
        ...
        if src_dev.type == "cpu" and self.device.type == "cuda":
            ...
            sample_td = sample_td.to(self.device, non_blocking=True)
        initial = {key: sample_td[key][:, 0] for key in self.LATENT_KEYS}
        data = sample_td[:, 1:]
        data.set_("action", sample_td["action"][:, :-1])
        index = [ind.view(-1, self.batch_length + 1)[:, 1:] for ind in info["index"]]
        return data, index, initial

    def update_factorized(self, index, fields):
        index = [ind.reshape(-1) for ind in index]
        target = self._buffer[index[1], index[0]]
        for key in self.LATENT_KEYS:
            value = fields[key].reshape(-1, *fields[key].shape[2:])
