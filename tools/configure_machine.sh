...
OMNINXT_ASSET_ROOT="${OMNINXT_ASSET_ROOT:-$ZYW_ROOT/omninxt_assets}"
...
Usage: tools/configure_machine.sh [options]
  ...
  --asset-root PATH        Machine-local model/scene assets
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --asset-root) OMNINXT_ASSET_ROOT="$2"; shift 2 ;;
...
OMNINXT_ASSET_ROOT="$(realpath -m "$OMNINXT_ASSET_ROOT")"
...
umask 077
{
  ...
  printf 'export OMNINXT_ASSET_ROOT=%q\n' "$OMNINXT_ASSET_ROOT"
