...
`--control-mode` 可替换为 `px4_classic`、`px4_ego`、`px4_navrl` 或
`px4_dpmpc`。episode 元数据会记录实际控制模式和基准算法标签；切换控制算法不改变
数据字段。与 `--benchmark-run` 同时使用时，录制开关会保留同步相机和真值深度链路；
未开启录制的纯基准任务仍会关闭这些渲染输出以节省 GPU。
