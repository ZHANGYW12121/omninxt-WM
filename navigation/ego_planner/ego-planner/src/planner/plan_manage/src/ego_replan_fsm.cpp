...
namespace ego_planner
{
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    case EXEC_TRAJ:
    {
      ...
      {
        // A sampled trajectory reaching its mathematical endpoint does not
        // prove that the PX4-controlled vehicle has tracked it.  This matters
        // especially when Isaac runs below real time.  Only declare mission
        // completion from the measured odometry; otherwise generate a fresh
        // trajectory from the actual state to the same retained target.
        if ((end_pt_ - odom_pos_).norm() < 0.5 && odom_vel_.norm() < 0.35)
        {
          have_target_ = false;
          changeFSMExecState(WAIT_TARGET, "FSM");
        }
        ...
        {
          ROS_WARN("Trajectory duration ended before vehicle settled at goal: distance=%.3f speed=%.3f; replanning from odometry.",
                   (end_pt_ - odom_pos_).norm(), odom_vel_.norm());
          changeFSMExecState(GEN_NEW_TRAJ, "FSM");
        }
        return;
      }
