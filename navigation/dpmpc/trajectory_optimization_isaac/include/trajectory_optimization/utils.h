...
inline double rpy_from_quaternion(geometry_msgs::Quaternion quat){
	// return is [0, 2pi]
...
inline void rpy_from_quaternion(geometry_msgs::Quaternion quat, double &roll, double &pitch, double &yaw){
	tf2::Quaternion tf_quat;
	tf2::convert(quat, tf_quat);
	tf2::Matrix3x3(tf_quat).getRPY(roll, pitch, yaw);
}
...
inline double getDistance(obstacle p1, pose p2){
    return sqrt(pow((p1.x - p2.x),2) + pow((p1.y - p2.y),2) + pow((p1.z - p2.z),2));    

}

inline double getDistance(pose p1, obstacle p2){
    return sqrt(pow((p1.x - p2.x),2) + pow((p1.y - p2.y),2) + pow((p1.z - p2.z),2));    

}


inline float my_logf (float);

/* compute inverse error functions with maximum error of 2.35793 ulp */
inline float my_erfinvf (float a)
{
    float p, r, t;
