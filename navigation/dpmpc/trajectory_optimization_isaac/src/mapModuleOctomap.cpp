...
mapModule::mapModule(const ros::NodeHandle& _nh): nh(_nh), tree_ptr(nullptr), abtree(nullptr){
	res = 0.1;
...
mapModule::mapModule(const ros::NodeHandle& _nh, double _res): nh(_nh), tree_ptr(nullptr), abtree(nullptr){
	res = _res;
	octomap_client = nh.serviceClient<octomap_msgs::GetOctomap>("/octomap_binary");
	xsize = 0.3; ysize = 0.3; zsize = 0.1;
}
...
mapModule::mapModule(const ros::NodeHandle& _nh, double _res, double _xsize, double _ysize, double _zsize):
	nh(_nh), tree_ptr(nullptr), abtree(nullptr){
	res = _res;
	octomap_client = nh.serviceClient<octomap_msgs::GetOctomap>("/octomap_binary");
...
void mapModule::clearMap(){
	delete tree_ptr;
	tree_ptr = new OcTree(res);
	abtree = tree_ptr;
}

void mapModule::loadOccupiedPoints(const std::vector<point3d>& points){
	clearMap();
	for (const point3d& point: points){
		tree_ptr->updateNode(point, true, true);
	}
	tree_ptr->updateInnerOccupancy();
}

bool mapModule::hasMap() const{
...
void mapModule::updateMap(){
	...
	if (tree_ptr == nullptr){
		abtree = nullptr;
		throw std::runtime_error("received OctoMap is not an OcTree");
	}
...
double mapModule::getMapResolution(){
	return tree_ptr == nullptr ? res : tree_ptr->getResolution();
}
