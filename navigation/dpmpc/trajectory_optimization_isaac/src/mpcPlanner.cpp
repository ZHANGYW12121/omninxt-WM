...
VariablesGrid mpcPlanner::getReference(const pose &p, int target_idx, double ratio){ // ratio: ratio of proceeding
	...
	for (int i=0; i<this->horizon; ++i){
		if (i < repeat_num){
			...
			int reference_idx = std::min(
				target_idx + count_proceed,
				static_cast<int>(this->ref_trajectory.size()) - 1
			);
			pose_i(0) = this->ref_trajectory[reference_idx].x;
			pose_i(1) = this->ref_trajectory[reference_idx].y;
			pose_i(2) = this->ref_trajectory[reference_idx].z;
			pose_i(3) = 0; // control thrust
			pose_i(4) = 0; // control roll
			pose_i(5) = 0; // control pitch
			++count_proceed;
...
pose mpcPlanner::getClosestObstaclePose(int start_idx, const obstacle &ob){
	...
	int count_idx = std::max(
		0,
		std::min(start_idx, static_cast<int>(this->ref_trajectory.size()) - 1)
	);
	double current_distance = 0;
	double prev_distance = std::numeric_limits<double>::infinity();
	while (count_idx < static_cast<int>(this->ref_trajectory.size())){
		...
		if (distance_change >= 0){
			return this->ref_trajectory[std::max(0, count_idx - 1)];
		}
		++count_idx;
		prev_distance = current_distance;
	}
	return this->ref_trajectory.back();
}
...
int mpcPlanner::optimize(const DVector &currentStates, double currentYaw, const std::vector<obstacle> &obstacles, DVector &nextStates, std::vector<pose> &mpc_trajectory, VariablesGrid &xd){ 
	...
	int solver_status = RTalgorithm.solve(0, currentStates);
